...
point cloud. The only requirement being that the operator uses a
technique called <<loop closure>>, where they must return to the exact
location where they started acquiring data. When using those scanners,
the devices produce two point clouds, one for the scene and one for
the trajectory that the operator took inside that point cloud.
